...
class default_loader
{
    ...
    static auto extract_line(const std::string &line, std::optional<std::size_t> expected_size = std::nullopt)
    {
        ...
        for(std::string cell; std::getline(linestream >> std::ws, cell, ','); )
        {
            ...
            cell.erase(std::find_if(cell.rbegin(), cell.rend(), [](auto c){ return !std::isspace(c); }).base(), cell.end());
...
private:
    ...
    struct masked_iterator
    {
        ...
        bool operator != (const masked_iterator & other) const { return iter != other.iter; }
        void operator ++ () { do { ++mask_it; ++iter; } while( iter != end_iter && !*mask_it ); }
